...
type PipelineDeal = { title: string; stage: string; value: number; currency: string };
...
export function DashboardClient({ initialData, displayName, userId }: DashboardClientProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [dealsRes, quotationsRes, leadsRes, eventsRes, todayEventsRes, leaderboardRes, countryRes, pipelineRes] = await Promise.all([
    ...
    setData({
      ...
      leaderboard: lb,
      byCountry: countries,
      pipelineDeals: (pipelineRes.data || []) as PipelineDeal[],
      pipelineTotal: pipelineTotalVal,
    });
    setRefreshing(false);
  }, [userId]);

  const { stats, leaderboard, byCountry, pipelineDeals, pipelineTotal } = data;
      ...
      <div className="px-6 py-5 max-w-6xl mx-auto space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            {[
              { label: "My Revenue This Month", value: formatCurrency(stats.revenue, "AED"), sub: null, icon: DollarSign, color: "bg-zinc-500" },
              { label: "My Meetings This Month", value: stats.meetingsThisMonth.toString(), sub: `${stats.meetingsCompleted} completed · 0.0/day avg`, icon: Calendar, color: "bg-zinc-700" },
              { label: "My Leads", value: stats.myLeads.toString(), sub: `${stats.leadsConverted} converted this month`, icon: Users, color: "bg-zinc-600" },
              { label: "My Deals", value: `${stats.openDeals} open`, sub: `AED 0 total · ${stats.dealsWon} won`, icon: Briefcase, color: "bg-zinc-800" },
              { label: "My Quotations", value: `${stats.quotationsSent} sent`, sub: `${stats.quotationsApproved} approved · 0% rate`, icon: FileText, color: "bg-zinc-800" },
              { label: "My Clients", value: stats.myClients.toString(), sub: `${stats.clientsRenewal} up for renewal`, icon: Building2, color: "bg-zinc-700" },
